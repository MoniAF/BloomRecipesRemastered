...
@section('title', 'Edit Recipe | Bloom Admin')

@section('content')
@php
    $ingredientNames = old(
        'ingredients',
        $ingredients->pluck('name')->all()
    );

    $amountValues = old(
        'amounts',
        $ingredients->pluck('amount')->all()
    );

    $unitValues = old(
        'units',
        $ingredients->pluck('measurement_units_id')->all()
    );
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <img
                    id="preview"
                    src="{{ asset('storage/imgs/' . $recipe->image) }}"
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    <label for="occasions_id" class="form-label">
                        Occasion
                    </label>
                    ...
                    >
                        @foreach ($levels as $level)
                            <option
                                value="{{ $level->id }}"
                                @selected(
                                    old('levels_id', $recipe->levels_id)
                                    == $level->id
                                )
                            >
                                {{ $level->level }}
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input
                        ...
                        type="number"
                        name="portions"
                        value="{{ old('portions', $recipe->portions) }}"
                        class="form-control"
                        min="1"
                        required
                    >
